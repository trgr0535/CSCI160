...
DigitalTimeException::DigitalTimeException(int _errornumber, string _errormessage)
{
	errorNumber = _errornumber;
	errorMessage = _errormessage;
}
string DigitalTimeException::getmessage(string _message)
{
	return errorMessage;
}
int DigitalTimeException::getnumber(int _number)
{
	return errorNumber;
}
DigitalTime::DigitalTime(int the_hour, int the_minute)
{
	...
	{
		throw DigitalTimeException(1, "Error Number 1");
	}
	else
	{
		hour = the_hour;
...
void read_minute(istream& ins, int& the_minute)
{
	...
	{
		throw DigitalTimeException(2, "Error number 2");
	}
	...
	{
		throw DigitalTimeException(3, "Error number 3");
	}
}
...
void read_hour(istream& ins, int& the_hour)
{
	...
	{
		throw DigitalTimeException(4, "Error number 4");
	}
	...
	else //(isdigit(c1) && isdigit(c2))
	{
		...
		if (c2 != ':')
		{
			throw DigitalTimeException(5, "Error number 5");
		}
	}

	if (the_hour < 0 || the_hour > 23)
	{
		throw DigitalTimeException(6, "Error number 6");
	}
}
